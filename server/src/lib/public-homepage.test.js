...
test('public homepage presents the brand and a real application journey', () => {
  ...
  assert.match(html, /在信仰里，认真靠近一个真实的人。/);
  ...
  assert.match(html, /rose-editorial-couple\.webp/);
...
test('public homepage styles avoid prohibited patterns', () => {
  ...
  assert.match(css, /--rose:\s*#fde8ee/i);
  assert.match(css, /--cherry:\s*#d9325d/i);
  assert.match(css, /--wine:\s*#521a2c/i);
  assert.match(css, /--mint:\s*#bfd8d0/i);
});
...
test('public homepage progressively enhances motion and meets contrast tokens', () => {
  ...
  assert.match(css, /--focus:\s*#521a2c/i);
  assert.match(js, /relationship-line/);
});

test('public homepage uses licensed East Asian documentary media with useful alternatives', () => {
  assert.match(html, /src="\.\/assets\/rose-editorial-couple\.webp"[^>]+alt="[^"]+"/);
  ...
  assert.match(attribution, /Daniel Case/);
