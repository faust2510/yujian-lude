...
export default function ChatMobile({ user, channels = [], active, messages = [], text = '', sending = false, loadingChannels = false, loadingMessages = false, error = '', bottomRef, onRetry, onTextChange, onSend, onBack }) {
  ...
  return (
    <section className="x-mobile-chat-thread">
      ...
      <div className="x-mobile-message-list">{loadingMessages ? <XMobileSkeleton lines={5} /> : messages.length === 0 ? <XMobileEmptyState title="发送第一封书信" /> : messages.map((message) => { const mine = message.sender_id === user?.id; return <div className={`x-mobile-message ${mine ? 'is-mine' : ''}`} key={message.id}><div>{message.body}</div>{message.created_at ? <small>{message.created_at}</small> : null}</div> })}<div ref={bottomRef} /></div>
